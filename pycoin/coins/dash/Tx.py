...
from pycoin.satoshi.satoshi_int import parse_satoshi_int, stream_satoshi_int
...
class Tx(BaseTx):
    ...
    def __init__(self, version, txs_in, txs_out, lock_time=0, unspents=None, extraPayload=None):
        """Tx constructor.
    ...
    def tx_outs_as_spendable(self, block_index_available=0):
        h = self.hash()
        return [
            self.Spendable.from_tx_out(
                tx_out, h, tx_out_index, block_index_available)
        ...
        for tx_out in self.txs_out:
            if tx_out.coin_value < 0 or tx_out.coin_value > self.MAX_MONEY:
                raise ValidationFailureError(
                    "tx_out value negative or out of range")
    ...
    def validate_unspents(self, tx_db):
        ...
        for h in tx_hashes:
            ...
            if the_tx.hash() != h:
                raise KeyError("attempt to load Tx %s yielded a Tx with id %s" % (
                    h2b_rev(h), the_tx.id()))
            tx_lookup[h] = the_tx
    ...
    # Could be improved in the future to support all special transaction data
    # https://docs.dash.org/en/stable/merchants/technical.html#v0-13-0-integration-notes
    # Example: https://github.com/DeltaEngine/NBitcoin/blob/master/NBitcoin.Altcoins/Dash.cs
